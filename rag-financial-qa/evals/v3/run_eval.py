...
from app.utils.financial_normalization import financially_equal, normalize_financial_value
...
def _score_output(output: dict[str, Any], truth: dict[str, Any], profile: str) -> dict[str, Any]:
    ...
    citation_valid = bool(verification.get("passed")) if profile == "verified_v3" and accepted else None
    return {
        "accepted": accepted,
        "refused": refused,
...
def _retrieval_recall(candidate_cases: list[dict[str, Any]], truths: list[dict[str, Any]]) -> float:
    ...
    for case, truth in zip(candidate_cases, truths):
        ...
        for context in case["contexts"][:5]:
            source_ok = expected_source in (None, "") or _norm(expected_source) in _norm(context.get("source"))
