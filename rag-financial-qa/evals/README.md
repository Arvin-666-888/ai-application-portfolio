...
这个目录的活动入口用于跨境电商商品事实 RAG：用固定问题集检查价格、库存数量、配送时长、关税税率的局部证据绑定，以及超出四类事实范围时的拒答。后半部分的金融 PDF Router、Gate 和冻结身份均为历史证据，继续保留原文与原口径。

## 评测集

- `fixtures/ecommerce_product_manual.txt`：SKU-A100 的商品身份、USD 价格和整数库存，事实绑定到平台、市场与生效日期。
- `fixtures/ecommerce_customs_compliance.txt`：SKU-C300 的 Shopee 新加坡进口关税税率合规记录。
- `fixtures/ecommerce_logistics_records.txt`：SKU-B200 的京东中国仓配送时长记录。
- `fixtures/ecommerce_product_facts.txt`：把多商品、多事实放在一起的综合 smoke fixture，仅用于快速链路冒烟，不是活动权威评测语料。
- `questions.jsonl`：当前活动集共 11 条，包含四类允许事实各 1 条正向题，以及资料外 SKU、超范围事实、多事实请求、换算/复杂公式和缺币种或单位的 7 条拒答。
- 活动 `run_eval.py`、`demo_e2e.py` 与 LangChain demo 只加载前三份权威 fixture，不加载综合 smoke fixture，避免重复事实污染检索和 citation 绑定。
- `fixtures/finance_summary_2024.txt`、`fixtures/risk_notice.txt`：历史金融 TXT fixture，仅保留兼容与历史证据，不再作为活动评测入口。

每条 case 建议包含：

- `category`：问题类别，例如 `ecommerce_price`、`ecommerce_logistics`、`ecommerce_guardrail`。
...
- `refusal_accuracy`：超出价格、库存数量、配送时长和关税税率范围的问题是否被拒答。
