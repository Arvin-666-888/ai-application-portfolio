...
INDEX_PROFILE_VERSION = "ecommerce-router-index-v1"
...
def parse_snapshot_path(document_id: int, pdf_sha256: str, attempt: int | None = None) -> Path:
    filename = f"{pdf_sha256[:12]}.json" if attempt is None else f"{pdf_sha256[:12]}-attempt-{attempt}.json"
    return Path(settings.DOCUMENT_PARSE_SNAPSHOT_DIR) / f"doc_{document_id}" / filename
...
def run_ingest(db: Session, job: DocumentJob) -> dict[str, Any]:
    ...
    if job.profile_version and job.profile_version != settings.PDF_PARSE_PROFILE:
        raise ValueError("document job parse profile no longer matches runtime configuration")
    ...
    snapshot = {
        ...
        "parse_profile": job.profile_version or settings.PDF_PARSE_PROFILE,
        "parse_result_schema": (
    ...
    snapshot_path = parse_snapshot_path(document.id, actual_sha, job.attempt_count)
...
def enqueue_finalize_if_ocr_terminal(db: Session, document_id: int) -> bool:
    ...
    if jobs and any(job.status in {"queued", "running", "stale"} for job in jobs):
        return False
    ingest_jobs = job_repository.list_document_jobs(
        db, document_id=document_id, job_type=INGEST_JOB_TYPE,
    )
    completed_ingest = next(
        (item for item in ingest_jobs if item.status == "completed" and (item.result or {}).get("snapshot_path")),
        None,
    )
    snapshot_path = (
        str(completed_ingest.result["snapshot_path"])
        if completed_ingest is not None
        else str(parse_snapshot_path(document.id, document.file_sha256))
    )
