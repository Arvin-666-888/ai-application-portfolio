...
VERIFIED_V3_PROMPT = """你只输出一个 JSON 对象，不输出 Markdown 或额外文字：
{
  ...
  "facts": [{
    "fact_type": "price、inventory_quantity、delivery_duration、customs_duty_rate 四选一",
    "value_text": "证据中的原始数值",
    "unit": "hour、day、business_day、percent 或 null",
    "currency": "CNY、USD、HKD 或 null",
    "sku": "证据中的 SKU 或 null",
    "product": "证据中的商品名或 null",
    "platform": "证据中的平台或 null",
    "market": "证据中的市场或 null",
    "date": "证据中的日期或 null",
    "citation_ids": ["C1"]
  }]
}
只能回答四类事实：明确货币的价格、整数库存数量、带小时/天/工作日单位的配送时长、带百分号的关税税率。只能使用 evidence 中存在的 citation ID。事实类型、数值、单位/币种、SKU、商品、平台、市场和日期必须由同一条证据中的同一表格行或同一句话支持；不得猜测缺失单位或币种，不得回答重量、尺寸或电气规格，不得计算或换算。资料不足时返回 {"answer_text":"根据现有资料无法回答该问题","facts":[]}。"""

VERIFIED_REFUSAL = "根据现有资料无法可靠回答该问题。系统未找到可同时验证商品事实、单位或币种、身份字段和引用的证据。"
...
def render_verified_answer(structured: StructuredAnswer) -> str:
    ...
    for fact in structured.facts:
        subject = " ".join(
            value for value in (
                fact.sku, fact.product, fact.platform, fact.market, fact.date, fact.fact_type
            ) if value
        )
...
async def retrieve_context(
    ...
    if settings.RETRIEVAL_PROFILE == "ecommerce_v2":
        return vector_store.query_ecommerce_v2(
            kb_id=kb_id,
...
async def prepare_question(
    ...
    refusal = ecommerce_guardrail_refusal(question)
    if refusal:
        return refusal, [], []
