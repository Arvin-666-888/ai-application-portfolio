...
PDF_ROUTING_POLICY_VERSION = "financial-pdf-routing-v1"
...
UNIT_TERMS = ("单位：元", "单位:元", "人民币元", "万元", "百万元", "亿元", "%", "％")
PERIOD_TERMS = ("本期", "上期", "本年", "上年")
...
ROUTING_POLICY: dict[str, Any] = {
    ...
    "requires_two_years_or_period_pair": True,
...
def page_features(
    ...
    *,
    ...
    return PDFPageProbe(
        ...
        period_hits=tuple(term for term in PERIOD_TERMS if term in text),
...
def classify(
    ...
    if _value(page, "table_title_hits", ()):
        reasons.append("financial_table_title")

    years = _value(page, "year_hits", ()) or ()
    periods = set(_value(page, "period_hits", ()) or ())
    has_comparison = len(years) >= 2 or (
        bool(periods & {"本期", "本年"}) and bool(periods & {"上期", "上年"})
    )
    if (
        ...
        reasons.append("numeric_financial_page")
...
def select(
    ...
    *,
    ...
    for page in pages:
        ...
        if "financial_table_title" in reasons:
    ...
    ranked = sorted(
        ...
        key=lambda page_number: (
            0 if "financial_table_title" in candidates[page_number] else 1,
