...
def test_completed_artifact_binds_statement_unit_and_column_scope(tmp_path):
    pdf_sha = "a" * 64
    artifact = build_completed_artifact(
        source="report.pdf",
        pdf_sha256=pdf_sha,
        ...
        result_payload={
            ...
            "page_count": 1,
            "parsing_res_list": [
                {"block_label": "paragraph_title", "block_content": "2024年度合并及公司利润表", "block_bbox": [1, 10, 20, 20], "block_order": 1},
                {"block_label": "text", "block_content": "金额单位为人民币千元", "block_bbox": [1, 21, 20, 30], "block_order": 2},
                {"block_label": "table", "block_content": "", "block_bbox": [1, 31, 20, 90], "block_order": 3},
            ],
            "table_res_list": [{
                "pred_html": (
                    "<table><tr><th>项目</th><th>2024年度 合并</th><th>2024年度 公司</th></tr>"
                    "<tr><td>营业收入</td><td>100</td><td>2</td></tr></table>"
                ),
                "table_ocr_pred": {"rec_texts": ["营业收入", "100", "2"]},
            }],
        },
        elapsed_seconds=1.0,
    )
    ...
    assert semantic["statement_type"] == "利润表"
    assert semantic["table_scope"] == "column-bound"
    assert semantic["unit"] == "千元"
    assert [item["scope"] for item in semantic["column_bindings"]] == ["合并", "母公司"]
