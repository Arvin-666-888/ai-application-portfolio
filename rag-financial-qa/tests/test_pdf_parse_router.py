...
def _financial_page(title: str = "") -> str:
    rows = [title, "项目 2024年 2023年", "营业收入 100 90"]
    rows.extend(f"财务指标{i} {i * 100} {i * 90}" for i in range(1, 14))
    return "\n".join(rows)
...
def test_feature_classification_and_selection_match_frozen_policy():
    pages = [
        _probe(1, "普通正文" * 30),
        _probe(2, _financial_page("合并利润表")),
        _probe(3, "短页"),
    ...
    assert "numeric_financial_page" in reasons
...
def test_router_uses_l2_table_and_skips_l3_narrative(tmp_path):
    ...
    router = PDFParseRouter(
        l2,
        l3,
        pdf_opener=lambda path: _PDF([_financial_page("合并利润表")]),
    )
