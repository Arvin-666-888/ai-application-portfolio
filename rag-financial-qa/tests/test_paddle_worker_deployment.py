...
def test_compose_declares_shared_container_namespace_and_blocks_host_worker():
    ...
    assert "PDF_PADDLE_ARTIFACT_DIR: /app/ocr_artifacts" in compose
    assert "./ocr_artifacts:/app/ocr_artifacts" in compose
    assert "environment: *rag-environment" in compose
    assert "volumes: *rag-volumes" in compose
