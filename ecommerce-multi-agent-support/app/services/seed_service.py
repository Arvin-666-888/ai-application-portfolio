...
SHOP_CONTEXTS = (
    ("shop-us", "US", "America/Los_Angeles", "USD"),
    ("shop-eu", "EU", "Europe/Berlin", "EUR"),
    ("shop-uk", "UK", "Europe/London", "GBP"),
)
...
def seed_demo_data(db: Session, *, seed: int) -> dict[str, int]:
    ...
    for index in range(1, 13):
        shop_id, market, shop_timezone, _ = SHOP_CONTEXTS[(index - 1) % len(SHOP_CONTEXTS)]
        users.append(
            UserTable(
    ...
    products_by_shop: dict[str, list[ProductTable]] = {context[0]: [] for context in SHOP_CONTEXTS}
    for index in range(1, 61):
        shop_id, _, _, currency = SHOP_CONTEXTS[(index - 1) % len(SHOP_CONTEXTS)]
        base_name, category, specifications = PRODUCT_TEMPLATES[(index - 1) % len(PRODUCT_TEMPLATES)]
        variant = (index - 1) // len(PRODUCT_TEMPLATES) + 1
        price = Decimal(str(round(39 + index * 5.15 + rng.uniform(-8, 8), 2)))
        product = ProductTable(
            shop_id=shop_id,
            sku=f"{shop_id.upper()}-{category[:3].upper()}-{index:04d}",
            name=f"VoltCore {base_name} V{variant}",
            category=category,
            price=price,
            currency=currency,
            stock=rng.randint(1, 180),
    ...
    for index in range(1, 101):
        ...
        shop_products = products_by_shop[user.shop_id]
        chosen_products = rng.sample(shop_products, k=rng.randint(1, 3))
        ...
        order = OrderTable(
            ...
            status=status,
            total_amount=total,
            currency=chosen_products[0].currency,
            created_at=created_at,
        )
        for product, quantity in zip(chosen_products, quantities):
            order.items.append(
                OrderItemTable(product_id=product.id, quantity=quantity, unit_price=product.price)
            )
        orders.append(order)
        db.add(order)
        db.flush()
