...
- 实现 SQL 安全控制：使用 `sqlglot` AST 限制单条只读 SELECT，拦截危险关键字、多语句、注释、会话变量赋值及已知副作用函数，并强制结果行数上限。
...
- 平台元数据默认使用 MySQL 8.0，并通过 SQLAlchemy Repository 和 FastAPI 依赖注入访问；pytest 使用隔离 SQLite 保证可重复执行。
- 内置财务经营样例数据仍使用 SQLite，生产业务数据源的多方言兼容需要按接入数据库分别验证。
- SQL 安全已升级为应用层 AST Guardrail；生产环境仍需要数据库只读账号、表/字段白名单、查询超时、审计日志、结果脱敏和资源配额。
